...
export default function ShopitoMartUpdate() {
   ...
   useEffect(() => {
    try {
       const fetchResources = async () => {
          const res = await fetch(`/api/order/orderListings?orderId=${orderId}`);
...
const handleSubmit = async (e) => {
  ...
  try {
     ...
     const res = await fetch(`/api/order/updateOrder/${formData._id}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="p-3 max-w-3xl mx-auto min-h-screen">
      <h1 className="text-3xl text-center my-7 font-extrabold underline text-blue-950 dark:text-slate-300">Update Orders</h1>
